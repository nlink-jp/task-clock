...
- **pause の永続化 (2026-08-31)**: 当初「runtime-only（再起動で解除）」としたが、GUI からの有効/無効コントロール要望（ユーザー）を受けて設計変更。config の `enabled` を GUI が書き換えるのは tasks.d 編集のスコープ外侵犯になるため、pause を SQLite（内部実装）に永続化し「デーモン所有の運用上の OFF スイッチ」に昇格。`enabled` = 宣言的基準（config）、`paused` = 運用状態（API/GUI）、という二層で責務が分かれる
...
- **実地事故: デーモン停止がタスクを殺す (2026-09-02)**: GUI アプリ終了でデーモンが道連れ停止し、実行中タスクが kill されて作業が失われた。根本原因は 2 層 — (1) GUI 同梱 CLI の `install` が `os.Executable()`（= .app 内部パス）を plist に書き、アプリのライフサイクルにデーモンが束縛された。対策: `install` はバイナリを `data_dir/bin/` へ自己コピーし plist はそこだけを指す。(2) 停止時に実行中タスクを kill していた。当初の修正案「殺さず手放すだけ」はユーザー指摘で棄却 — 復旧したデーモンが orphan を知らずに次発火を実行すると**二重実行**になる。確定設計: ReleaseAll は run 行を開いたまま released_runs 台帳（pid/argv0/時刻）へ記録し、次のデーモンが生存 + 同一性（ps 照合）を確認して adopt、overlap ポリシーを orphan に対して維持する。adopted run にも timeout / log_max_mb / kill-and-restart は pgid シグナルで効くが、exit code は原理的に取得不能（履歴は「exit status unknown」で閉じる）。停止をまたげない queue 済み発火は `missed(daemon_stop)` として記録。「デーモンがタスクを殺すのは明示設定した箇所のみ」を不変条件に昇格
- **start/stop の追加 — 動作状態とセットアップの分離 (2026-09-02)**: 同事故のフォローアップでユーザーが指摘 — GUI の電源スイッチが「launch agent 登録」と「動作状態」を 1 つに畳んでおり、止めるためにアンインストールする構造だった。CLI に `start`/`stop` を追加して分離。stop は `launchctl disable` + `bootout`（disable が無いと RunAtLoad が次回ログインで意図に反して蘇らせる — スイッチ = 永続的な意図、の原則）、start と install は `enable` で解除。stop は release セマンティクスに乗るため実行中タスクを殺さない
- **リリース前検証パスによる設計強化 (2026-09-02)**: 独立検証（org 規範）が adoption 設計の欠陥 4 件を確認し、以下に改訂 — (1) 同一性照合を argv0 部分一致から**プロセス開始時刻**へ（run 内の exec で偽陰性→二重実行、pid 再利用で偽陽性→無関係プロセスへの kill、の両方向を排除; シグナル送信前にも再検証）。(2) 台帳を停止時書き込みから **spawn 時書き込みの live-run レジストリ**へ — クラッシュ（KeepAlive が数秒で再起動する）でも adopt が成立し、停止順序の競合も構造的に消滅。(3) adopted run の kill に TERM→KILL エスカレーション（TERM 無視プロセスによる busy 永久化を防止）。(4) 「exit 不明」は exit_code NULL で記録（不明≠失敗）+ FinishRun に open-row ガード（停止窓で記録された実 exit を後続 finalize が上書きしない）
